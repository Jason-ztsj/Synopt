...
> **关于vibe coding**
> 这是一个与 AI 深度协作、以vibe coding（氛围编程）方式构建的项目：代码的大部分由 AI 生成，作者负责立方向、做架构与安全决策，并逐一审查验证。这是有意的、现代的工程选择，也是对本项目来历的诚实交代——仓库里是真实可运行、带完整单元与集成测试的实现，不是演示稿。AI 承担了大量编码，而"同见"的定位、取舍与设计判断始终由人掌握。
> 如果你因"AI 生成"而对质量存疑：欢迎自己跑测试、读代码去验证——一个诚实的出处，不该成为被嫌弃的理由。
...
应用使用 Node.js 内置 `node:sqlite`，无需安装 SQLite 服务或编译原生数据库扩展。MathLive 和 KaTeX 都由 npm 安装并从本机提供，浏览器运行时不访问 CDN。它支持常见的 AMD64 和 ARM64 Linux 主机，并已在 ARM 开发板上验证可正常运行。
...
打开 `http://127.0.0.1:3000`。应用进程会监听 `0.0.0.0`，这是为了方便无显示器机器通过局域网调试；本机仍可用回环地址访问。应用启动时会创建数据目录并执行向后兼容的数据库迁移，不会删除既有视频或讨论。
...
`docker compose down` 不会删除 `./data`。镜像基于多架构 `node:24-bookworm-slim`，可直接在 AMD64 或 ARM64 主机上构建（已在 ARM 开发板上验证）。若要在另一台机器显式构建 ARM64 镜像：

```bash
docker buildx build --platform linux/arm64 -t synopt-video-mvp:arm64 --load .
```

这里的 `synopt-video-mvp` 只是本地镜像标签，并非正式英文产品名。`app` 镜像与 Compose 服务使用 `/healthz` 健康检查；Web 服务健康后该接口返回 HTTP 200，validator 通过退出码和日志反映状态。

## 局域网调试

直接运行 Node 时，应用已经监听 `0.0.0.0`。在本机启动后，同一局域网的其他设备可打开 `http://<本机局域网地址>:3000`。

使用 Compose 时，`HOST_BIND_ADDRESS` 决定宿主机在哪些地址发布端口。可先用下面的命令找到本机地址：
