...
- 从旧 schema 升级当前 v7 前也应按上述方式制作一致备份：v5 引入 CMS，v6 扩展媒体兼容度，v7 迁移三档视频价值评价。程序不提供自动降级，回滚旧代码需要恢复升级前备份。

## 当前限制

- HEVC 只使用浏览器/系统原生能力；尚无 hevc.js、WebCodecs、DASH/HLS 或备用编码。
- `BufferTarget` 会让浏览器在规范化阶段同时持有源文件和目标文件；即使网络传输随后分片，当前默认 1024 MiB 上限仍可能使低内存手机失败。
- 约 50 MiB 以上的规范化资产会使用应用层分片上传，默认每片 16 MiB，以绕开隧道单请求体上限。会话注册表仍在单进程内存中、30 分钟过期，分片暂存在磁盘；它支持同一会话内重传分片，但不支持进程重启后的断点续传。
- 没有清晰度梯度、缩略图、响度规范化、转码、跨进程断点续传或多实例共享上传会话。
...
集成测试需要 FFmpeg/ffprobe，并覆盖真实 H.264/AAC MP4、VP8/VP9/AV1 策略、VP9/Opus WebM、普通与分片上传、正确 `ftyp` 后接垃圾、隔离区权限、动态 MIME、Range、讨论和进程重启。2026-08-27 的 Node 24 隔离验收中，完整集成测试为 24/24。
