...
CMS 工作队列仍使用 `created_at DESC, id DESC` 的确定性顺序，后台不提供排序算法调参能力。公开目录已经由 `src/ranking.js` 的透明排序 v1 驱动，并由 `/algorithm`、`docs/algorithm.md` 和离线不变量校验公开说明；它与治理决定保持分离。未来若出现第二种可发布算法，再扩展“算法版本—离线校验—预览—发布—公开说明—回滚”链路。
...
讨论目标的利益相关者同时包括讨论作者和承载视频作者；任一身份都不能调查、处理或复核该讨论。视频永久删除会在内部保留承载视频作者关联，只在公开映射中匿名化，避免删除后再建立讨论案件绕过回避边界。
...
## 4. CMS schema v5（当前整体 schema v7）

CMS 的表和字段在 v5 引入：迁移只增加字段、表、索引和只追加保护触发器，保留 v4 数据。旧讨论迁移后为 `visible`，所有治理版本从 0 开始。当前数据库整体版本为 v7；后续 v6 媒体兼容度和 v7 三档价值投票迁移不改变下述治理表语义。
...
启动迁移在事务中执行并在提交前运行 `PRAGMA foreign_key_check`。生产升级前应停止写流量并复制整个 `data` 目录（包括 SQLite 主文件及同目录 WAL/SHM），或使用 SQLite 在线备份命令生成一致备份；升级后核对 `PRAGMA user_version = 7`、外键检查、管理员数量和队列状态。旧部署可继续通过 `DATABASE_PATH=./data/gongying.sqlite` 使用原文件；新默认名为 `synopt.sqlite`。Compose 会传入该变量，不能只改文件名而不迁移实际数据库。程序不自动降级；回滚代码前应恢复升级前一致备份。
...
自动测试覆盖 v0/v1/v3/v4→v7 迁移、案件目标与唯一性、外键、只追加触发器，以及举报去重、案件 CAS、视频/讨论决定、通知、最后管理员保护、暂停会话、申诉回避与撤销冲突、标签合并、验证重试和删除后利益冲突保留。EJS 渲染测试检查版本、理由、私密媒体和管理员界面；真实 HTTP 流程覆盖成员/审核员/管理员、再认证、CSRF、举报冷却、暂停账号、短时媒体授权、普通与分块上传、媒体验证、Range 和进程重启。2026-08-27 的 Node 24 + FFmpeg 隔离验收结果为：单元测试 153/153、集成测试 24/24、透明排序离线不变量 6/6。
