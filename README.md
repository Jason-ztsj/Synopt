...
- 用户名、显示名称和密码注册；账号菜单提供公开资料、我的稿件、我的讨论、通知、设置和退出入口。
...
- 视频、讨论和回复都有独立的“认同/反对”计数；一个账号对同一目标只有一个可取消、可切换的选择。
- 作者可以修改自己的讨论标题和正文，页面公开显示修改次数与最后修改时间；删除有回复的内容时只留下匿名墓碑，不会连带删除他人的回复。
- “我的稿件”支持可逆撤回和重新发布；永久删除必须先撤回并输入完整标题。文件删除先持久入队再立即尝试，失败由验证器退避重试；只有删除前真正公开、验证通过且未被审核隐藏的稿件，才在原链接保留只读讨论档案。
- 站内通知支持回复、稿件认同/反对和预留的系统消息；未读投票按作品、方向与不同账号的净投票状态聚合，取消再重投不会刷高计数。页面打开时每 30 秒更新一次未读数。
...
- SQLite 保存账号、会话、通知偏好、通知、视频元数据和讨论；视频、封面和头像文件分别保存在数据目录中。
...
- 当前没有邮箱验证、密码找回、多因素认证、会话列表、封禁、审核后台、配额或内容举报。
- 修改密码会撤销其他设备上的会话；账号删除不可恢复，可选择永久删除本人稿件和按墓碑规则删除本人讨论，未选择的内容会匿名保留。
- 当前局域网 HTTP 不是安全上下文，只提供站内通知；页面打开期间的浏览器系统通知必须经用户授权并通过 HTTPS 访问。关闭网页后的 Web Push 尚未实现。
- 账号、讨论和图片处理冷却都在单进程内存中，重启后清空，多实例之间也不共享。图片规范化另有全局并发上限，满载时会立即返回可重试错误。
...
测试覆盖配置与输入边界、密码和安全令牌、头像/封面真实解码与去元数据、图片并发闸门、数据库迁移、持久删除队列、许可证规范化、Markdown/XSS、客户端 IP、冷却窗口、媒体状态机和真实 FFmpeg 验证，以及真实 HTTP 账号资料、内容撤回/删除、讨论墓碑、通知、注销、MP4/WebM 上传、伪文件头拒绝、Range 和讨论流程。图片与媒体相关测试会调用本机的 `ffmpeg`，媒体测试还会调用 `ffprobe`。
...
平台信息架构、横屏长视频约束、透明排序以及未来 CMS 的数据边界见 [`docs/platform-architecture.md`](docs/platform-architecture.md)。
...
8. 视频和讨论的认同/反对可以切换和取消，同一账号不会产生重复票。
...
注册和登录并不使当前 MVP 适合实际运营。公开部署仍可能遭遇违法或侵权内容、垃圾账号、密码攻击、恶意文件、流量滥用和磁盘耗尽。当前也没有邮箱所有权验证、密码重置、账号恢复、审核、封禁、存储配额、备份保留策略或事件响应流程。已有的作者自助删除不能替代平台治理、法务删除或管理员审计能力。
...
SQLite 持久化用户名、显示名称、简介、头像引用、带盐密码摘要、会话/CSRF 令牌摘要、通知偏好与未读消息、视频元数据、验证状态/摘要/SHA-256，以及讨论的账号关联、显示名称快照、Markdown 原文和时间；视频、封面与头像二进制文件单独保存。浏览器上报的源容器和编码只用于诊断，最终值来自服务端探测。应用只在内存中短暂使用客户端 IP 做注册、登录和讨论冷却，不把 IP 写入数据库或页面。
